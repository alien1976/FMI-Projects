...
void Lists::deleteResize(int index)
{
...
void Lists::setData(char * buff, char *name, int beg, int end)
{
	char new_name[20] = { 0 };
	int i = 0;
...
void Lists::saveData(std::ofstream & file)
{
	int cnt = 0;
...
void Lists::erase(char * path, int cnt_tags, int beg, int alltags)
{
	int cnt_erased = 0;
	cnt_tags++;
...
void Lists::findIdAdd(char * id, char *content)
{
	for (int i = 0; i < cnt_tags; i++)
	{
		if (!tags[i]->getId())
...
void Lists::deleteBuffer(int index)
{
	int cnt = 0;
	int i = 0;
	while (buff[i] != '\0')
...
void Lists::resizeBuffer(char * content, int &end_buff,int number_more)
{
...
void Lists::setContent(char * content)
{
...
void Lists::createContent(char * path, int cnt_tags_path, int beg, int alltags)
{
	cnt_tags_path++;
	int cnt_real = this->cnt_tags;
...
void Lists::create(char * content)
{
	int end_buff = strlen(buff);
	int cnt_tabs = cntTabs();
	resizeBuffer(content, end_buff, cnt_tabs + 2);
...
void Lists::within(char * path, int cnt_tags, int beg, int alltags)
{
	cnt_tags++;
	int cnt_real = this->cnt_tags;
	char *tag = getTagFromPath(path, cnt_tags, beg);
