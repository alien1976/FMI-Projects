...
void Basic::deleteResize(int index)
{
...
void Basic::setData(char * buff, char *name, int beg, int end)
{
...
void Basic::saveData(std::ofstream & file)
{
	int cnt = 0;
...
void Basic::erase(char * path, int cnt_tags, int beg, int alltags)
{
	int cnt_erased = 0;
	cnt_tags++;
...
void Basic::findIdAdd(char * id, char *content)
{
	for (int i = 0; i < cnt_tags; i++)
	{
		if (!tags[i]->getId())
...
void Basic::deleteBuffer(int index)
{
	int cnt = 0;
	int i = 0;
	while (buff[i] != '\0')
...
void Basic::resizeBuffer(char * content, int &end_buff, int number_more)
{
...
void Basic::setContent(char * content)
{
...
void Basic::createContent(char * path, int cnt_tags_path, int beg, int alltags)
{
	cnt_tags_path++;
...
void Basic::create(char * content)
{
	int end_buff = strlen(buff);
	int cnt_tabs = cntTabs();
	resizeBuffer(content, end_buff, cnt_tabs + 2);
...
void Basic::within(char * path, int cnt_tags, int beg, int alltags)
{
	cnt_tags++;
	int cnt_real = this->cnt_tags;
	char *tag = getTagFromPath(path, cnt_tags, beg);
